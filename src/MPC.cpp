...
AD<double> polyeval_D(Eigen::VectorXd coeffs, AD<double> x) {
	AD<double> result = 0.0;
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // the Solver function below.
	  ...
	  // Set starting values for constraints
	  ...
	  for (unsigned int t = 1; t < N; t++) {
		  ...
		  AD<double> psides0 = CppAD::atan(polyeval_D(coeffs, x0));
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Set the values for the initial state
 /* vars[x_start] = x;
  vars[y_start] = y;
  vars[psi_start] = psi;
  vars[v_start] = v;
  ...
  for (unsigned int i = 0; i < N; i++) {
	  sol_vector.push_back(solution.x[x_start + i]);
	  sol_vector.push_back(solution.x[y_start + i]);
  }
